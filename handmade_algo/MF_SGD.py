...
def matrix_factorization_SGD(train, test, gamma=0.01, n_features=20, n_iter=20):
    ...
    # init matrix
    user_features, item_features = init_MF(train, num_features, 'global_mean')
...
def matrix_factorization_SGD_full_training(data, gamma=0.01, n_features=20, n_iter=20):
    """matrix factorization by SGD."""
    # define parameters
    gamma = gamma
    num_features = n_features  # K in the lecture notes
    ...
    # init matrix
    user_features, item_features = init_MF(data, num_features, 'global_mean')
...
def predict(item_features, user_features, path_test_set, filename="pred_MF_SGD"):
    ...
    with open(filename + '.csv', 'w') as file:
        ...
        for row, col, rating in zip(users, items, ratings):
            r = str(row + 1)
            c = str(col + 1)
