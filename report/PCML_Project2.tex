...
In order to create our recommender algorithm, we followed 2: 
\begin{enumerate}
...
\item Optimizing the weights for each model prediction produced by the previous step, by running a SLSQP (as explained in section \ref{SLSQP}) optimization method and by computing a 5-folds Cross-Validation score at each step of the algorithm. 
\end{enumerate}
...
as much as possible the RMSE, while at the same time introducing a slight overfitting of the
...
The optimization choice also allows negative weights. We decided not to put any contraint about that
for two reasons:
\begin{itemize}
